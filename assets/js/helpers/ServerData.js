  ...
  async maintenancesByVehicleID(vehicleId) {
    try {
      ...
      return {
        ...
        data: maintenances
          .map(({
            id, works, engine, transmission, wheelDrive,
          }) => {
            ...
            return {
              ...
              works: works.map((work) => {
                const {
                  id, name, period: repeat, recommended, description: note, position, parts, price,
                } = work;
                return {
                  id,
                  name,
                  ...
                  parts: parts.map((part) => {
                    const {
                      quantity,
                      part: {
                        id, name, unit, price, manufacturer: { name: manufacture },
                      },
                    } = part;
                    return {
                      id,
  ...
  async getVehicleByID(vehicleId) {
    try {
      ...
      return {
        response: 200,
        data: {
          id, caseName, name, manufacturer, yearFrom, yearTill,
        },
      };
    } catch (e) {
      return {
