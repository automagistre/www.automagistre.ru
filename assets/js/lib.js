...
export const mobChecker = (maxWinWidth) => /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(
  navigator.userAgent,
)
...
const startParallax = (element) => {
  ...
  const secTop = element.getBoundingClientRect().top + pageYOffset;
  let startLevel;
  let stopLevel;
...
export const initParallaxAnimation = (features, images) => {
  if (!mobChecker(1024) && features) {
    const featuresParr = () => {
      const thisOffset = startParallax(features);
      if (thisOffset) {
        for (const image of images) {
          gsap.to(image.node, {
            duration: 2, y: thisOffset / image.position, force3D: true, delay: 0.1,
          });
        }
      }
    };
...
export const updateOdometerData = async (node) => {
  ...
  const vehiclesOdometerNode = node.querySelector('[data-type="vehicles"]');
...
export const initTabs = (tabsID, bodyID) => {
  ...
  const bodySec = document.querySelector(`#${bodyID}`);
  const tabsBtns = tabSec.querySelectorAll('.js-tabs-btn');
  const bodyItems = bodySec.querySelectorAll('.js-tabs-item');
